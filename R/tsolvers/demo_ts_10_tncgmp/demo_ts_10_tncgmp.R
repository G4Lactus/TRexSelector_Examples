...
demo_early_stopping <- function(high_dim, rnd_coef, T_stop) {
  ...
  solver <- TNCGMP_Solver$new(dat$X, dat$D, dat$y, variant = "line_search", normalize = TRUE,
...
demo_with_external_normalizer <- function(high_dim, rnd_coef, T_stop) {
  ...
  solver <- TNCGMP_Solver$new(dat$X, dat$D, dat$y, variant = "line_search", normalize = FALSE,
...
demo_serialization <- function() {
  ...
  solver_ref <- TNCGMP_Solver$new(dat$X, dat$D, dat$y, variant = "line_search", normalize = TRUE,
                                  intercept = TRUE, verbose = TRUE)
  solver_ref$execute_step(T_stop_final, early_stop = TRUE)
  cat(sprintf("\u2713 Reference completed with %d steps\n\n",
              solver_ref$get_num_steps()))

  filename <- file.path(this_dir_, "tncgmp_checkpoint.bin")

  # STEP 1: Run partial path and save checkpoint.
  solver1 <- TNCGMP_Solver$new(dat$X, dat$D, dat$y, variant = "line_search", normalize = TRUE,
                               intercept = TRUE, verbose = TRUE)
  ...
  solver2 <- TNCGMP_Solver$new(dat$X, dat$D, dat$y, variant = "line_search", normalize = TRUE,
                               intercept = TRUE, verbose = TRUE)
...
demo_memory_mapped <- function(high_dim, rnd_coef, T_stop) {
  ...
  X_file <- file.path(this_dir_, "demo_tncgmp_X.bin")
  ...
  cat("Running T-NCGMP on memory-mapped data...\n")
  solver <- TNCGMP_Solver$new(X_mmap, dat$D, dat$y, variant = "line_search", normalize = TRUE,
                              intercept = TRUE, verbose = TRUE)
  solver$execute_step(T_stop, early_stop = TRUE)
  cat("\u2713 T-NCGMP completed\n\n")
