...
demo_early_stopping <- function(high_dim, rnd_coef, T_stop) {
  ...
  solver <- TGP_Solver$new(dat$X, dat$D, dat$y, normalize = TRUE,
...
demo_with_external_normalizer <- function(high_dim, rnd_coef, T_stop) {
  ...
  solver <- TGP_Solver$new(dat$X, dat$D, dat$y, normalize = FALSE,
...
demo_serialization <- function() {
  ...
  solver_ref <- TGP_Solver$new(dat$X, dat$D, dat$y, normalize = TRUE,
  ...
  filename <- file.path(this_dir_, "tgp_checkpoint.bin")

  # STEP 1: Run partial path and save checkpoint.
  solver1 <- TGP_Solver$new(dat$X, dat$D, dat$y, normalize = TRUE,
                            intercept = TRUE, verbose = FALSE)
  ...
  solver2 <- TGP_Solver$new(dat$X, dat$D, dat$y, normalize = TRUE,
                            intercept = TRUE, verbose = FALSE)
...
demo_memory_mapped <- function(high_dim, rnd_coef, T_stop) {
  ...
  X_file <- file.path(this_dir_, "demo_tgp_X.bin")
  ...
  cat("Running T-GP on memory-mapped data...\n")
  solver <- TGP_Solver$new(X_mmap, dat$D, dat$y, normalize = TRUE,
                           intercept = TRUE, verbose = TRUE)
  solver$execute_step(T_stop, early_stop = TRUE)
  cat("\u2713 T-GP completed\n\n")
