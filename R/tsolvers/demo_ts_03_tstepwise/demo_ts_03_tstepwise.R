...
demo_early_stopping <- function(high_dim, rnd_coef, T_stop) {
  ...
  true_coefs <- if (rnd_coef) c(-0.4, -0.2, -0.8, 1.1, 2.5) else c(0.5, 1, 1, 1, 0.5)
  snr <- 0.1
  ...
  solver <- TSTEPWISE_Solver$new(dat$X, dat$D, dat$y, normalize = TRUE,
...
demo_with_external_normalizer <- function(high_dim, rnd_coef, T_stop) {
  ...
  solver <- TSTEPWISE_Solver$new(dat$X, dat$D, dat$y, normalize = FALSE,
...
demo_serialization <- function() {
  ...
  solver_ref <- TSTEPWISE_Solver$new(dat$X, dat$D, dat$y, normalize = TRUE,
  ...
  filename <- file.path(this_dir_, "tstepwise_checkpoint.bin")

  # STEP 1: Run partial path and save checkpoint.
  solver1 <- TSTEPWISE_Solver$new(dat$X, dat$D, dat$y, normalize = TRUE,
                                  intercept = TRUE, verbose = FALSE)
  ...
  solver2 <- TSTEPWISE_Solver$new(dat$X, dat$D, dat$y, normalize = TRUE,
                                  intercept = TRUE, verbose = FALSE)
...
demo_memory_mapped <- function(high_dim, rnd_coef, T_stop) {
  ...
  X_file <- file.path(this_dir_, "demo_tstepwise_X.bin")
  ...
  cat("Running T-Stepwise on memory-mapped data...\n")
  solver <- TSTEPWISE_Solver$new(X_mmap, dat$D, dat$y, normalize = TRUE,
                                 intercept = TRUE, verbose = TRUE)
  solver$execute_step(T_stop, early_stop = TRUE)
  cat("\u2713 T-Stepwise completed\n\n")
