...
```r
solver <- TSTAGEWISE_Solver$new(X, D, y, normalize = TRUE, intercept = TRUE, verbose = TRUE)
...
After execution, the demo prints the selected variables, selection-quality diagnostics, and a T-Stagewise-specific diagnostics line with the number of removals and the cycling ratio (`get_num_removals()` / `get_cycling_ratio()`).

### 2. External normalization

The second part applies external L2 normalization via `LpNormScaler` to both the signal matrix and the dummy matrix, centers the response manually, and then runs T-Stagewise with `normalize = FALSE` and `intercept = FALSE`.
...
The third part runs a reference fit to a final stopping level, then separately runs a shorter partial fit, saves the solver state to `tstagewise_checkpoint.bin` in the demo folder, reloads it, and continues from the checkpoint to the same final stopping level.
...
The reloaded run is compared against the reference path using the final number of steps, the final RSS difference, the final $R^2$ difference, and equality of the recorded action path (no removal/cycling diagnostics line in this part). The checkpoint file is removed afterward.

### 4. Memory-mapped data

The fourth part writes the design matrix $X$ to disk via `convert_to_memory_mapped()` and runs T-Stagewise on the resulting `mmap_matrix` view. The R binding accepts a memory-mapped $X$ while $D$ and $y$ stay in memory; the cpp demo maps all three.

The cpp counterpart generates $p = 500{,}000$ predictors directly on disk; the R port scales this down to $p = 5{,}000$ (with $n = 1{,}000$ and $10p$ dummies). The file `demo_tstagewise_X.bin` is written into the demo folder and removed at the end. The removals and cycling-ratio diagnostics are printed here as well.
...
```bash
Rscript R/tsolvers/demo_ts_05_tstagewise/demo_ts_05_tstagewise.R
```
...
- The removals and cycling-ratio diagnostics line is printed after parts 1, 2, and 4 (not after the serialization part).
- The serialization example writes a temporary checkpoint file named `tstagewise_checkpoint.bin` and removes it afterward.
- The memory-mapped example writes a temporary file for $X$ only, runs T-Stagewise on the mapped view, and removes the file at the end.
