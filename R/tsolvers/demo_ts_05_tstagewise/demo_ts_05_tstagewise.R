...
demo_early_stopping <- function(high_dim, rnd_coef, T_stop) {
  ...
  solver <- TSTAGEWISE_Solver$new(dat$X, dat$D, dat$y, normalize = TRUE,
  ...
  cat(sprintf("Diagnostics: removals = %d, cycling ratio = %.4f\n",
              solver$get_num_removals(), solver$get_cycling_ratio()))
  cat("\n\n")
}
...
demo_with_external_normalizer <- function(high_dim, rnd_coef, T_stop) {
  ...
  solver <- TSTAGEWISE_Solver$new(dat$X, dat$D, dat$y, normalize = FALSE,
...
demo_serialization <- function() {
  ...
  solver_ref <- TSTAGEWISE_Solver$new(dat$X, dat$D, dat$y, normalize = TRUE,
  ...
  filename <- file.path(this_dir_, "tstagewise_checkpoint.bin")

  # STEP 1: Run partial path and save checkpoint.
  solver1 <- TSTAGEWISE_Solver$new(dat$X, dat$D, dat$y, normalize = TRUE,
                                   intercept = TRUE, verbose = FALSE)
  ...
  solver2 <- TSTAGEWISE_Solver$new(dat$X, dat$D, dat$y, normalize = TRUE,
                                   intercept = TRUE, verbose = FALSE)
...
demo_memory_mapped <- function(high_dim, rnd_coef, T_stop) {
  ...
  X_file <- file.path(this_dir_, "demo_tstagewise_X.bin")
  ...
  cat("Running T-Stagewise on memory-mapped data...\n")
  solver <- TSTAGEWISE_Solver$new(X_mmap, dat$D, dat$y, normalize = TRUE,
                                  intercept = TRUE, verbose = TRUE)
  solver$execute_step(T_stop, early_stop = TRUE)
  cat("\u2713 T-Stagewise completed\n\n")
