...
In the demo, the solver is an R6 class from the TRexSelectorNeo package, constructed with the adaptivity parameter `rho = 1.0` and advanced through calls of the form

```r
solver <- TAFS_Solver$new(X, D, y, rho, normalize = TRUE, intercept = TRUE, verbose = TRUE)
...
The second part applies external L2 normalization via `LpNormScaler` to both the signal matrix and the dummy matrix, centers the response manually, and then runs T-AFS with `normalize = FALSE` and `intercept = FALSE`.
...
The third part runs a reference fit to a final stopping level, then separately runs a shorter partial fit, saves the solver state to `tafs_checkpoint.bin` in the demo folder, reloads it, and continues from the checkpoint to the same final stopping level.

Unlike the cpp version, which uses a static `load(filename, X, D)` factory, the R binding hydrates an existing solver instance: a fresh solver is constructed with the same `X`, `D`, `y`, and `rho`, and the checkpoint is loaded via `solver$load(path)`.
...
The fourth part writes the design matrix $X$ to disk via `convert_to_memory_mapped()` and runs T-AFS on the resulting `mmap_matrix` view. The R binding accepts a memory-mapped $X$ while $D$ and $y$ stay in memory; the cpp demo maps all three.

The cpp counterpart generates $p = 500{,}000$ predictors directly on disk; the R port scales this down to $p = 5{,}000$ (with $n = 1{,}000$ and $10p$ dummies). The file `demo_tafs_X.bin` is written into the demo folder and removed at the end.
...
```bash
Rscript R/tsolvers/demo_ts_12_tafs/demo_ts_12_tafs.R
```
...
- All parts of this demo construct the solver with `rho = 1.0`.
...
- The serialization example writes a temporary checkpoint file named `tafs_checkpoint.bin` and removes it afterward.
- The memory-mapped example writes a temporary file for $X$ only, runs T-AFS on the mapped view, and removes the file at the end.
