...
trx_00_gauss_data <- function() {
  ...
  ctrl     <- trex_control(solver = "TLARS", K = PARAMS$K)
  selector <- TRexSelector$new(
    X       = X,
    y       = y,
    tFDR    = PARAMS$tFDR,
    seed    = PARAMS$seed, # using a seed != -1 for reproducibility
    verbose = TRUE,
    control = ctrl
  )
  selector$select()
  ...
  .print_result(
    "Part 0 \u2014 Gauss data  [classical T-Rex, TLARS]",
...
if (run_part_00) trx_00_gauss_data()
cat("\nGauss data demo complete.\n")
