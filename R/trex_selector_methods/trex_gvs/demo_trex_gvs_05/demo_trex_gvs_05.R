...
CFG <- list(
  ...
  df       = 3L,           # Student-t degrees of freedom
...
#' Run one MC point on the t3 equi-blocks DGP for a given (method, snr, rho).
.t3_point <- function(method, snr, rho) {
  .run_mc_t3_equi_blocks(
    ...
    df = CFG$df, en_solver = method$en_solver, n_cores = num_cores)
}
...
if (run_part_1) {
  cat("\n", strrep("=", 70), "\n", sep = "")
  cat("Heavy-Tailed Equi-Blocks GVS  |  Part 1: Single-run\n")
  ...
  dat_p1 <- dgp_t3_equi_blocks_snr(n = CFG$n, p = CFG$p, snr = CFG$snr,
  ...
  cat(sprintf("[Part 1] Max |X| (Gaussian ~ 3.0, t(%d) should be larger): %.4f\n",
              CFG$df, max(abs(dat_p1$X))))
  ...
  cat(sprintf("[Part 1] Cor(X[,b1[1]], X[,b1[2]]) [expect rho=%.2f]: %.4f\n\n",
              CFG$rho, cor(dat_p1$X[, b1_idx[1L]], dat_p1$X[, b1_idx[2L]])))
  ...
  for (m in GVS_METHODS) {
    ...
    .print_result(sprintf("Part 1 — HT Equi-Blocks GVS  [%s]", m$label),
                  dat_p1, res, CFG$tFDR)
  }
}
...
# ==============================================================================
# Fixed rho = 0.75. Compares EN / EN+AUG / IEN.

if (run_part_2) {
  snr_grid <- c(0.1, 0.2, 0.5, 1.0, 2.0, 5.0)

  cat(strrep("=", 70), "\n")
  cat("HT Equi-Blocks GVS MC — Part 2: SNR sweep\n")
  cat(sprintf("tFDR=%.2f  rho=%.2f  t(%d)  n=%d  p=%d  %d MC\n",
              CFG$tFDR, CFG$rho, CFG$df, CFG$n, CFG$p, CFG$num_MC))
  cat(strrep("=", 70), "\n\n")
  ...
  for (m in GVS_METHODS) {
    ...
    res <- lapply(snr_grid, function(s) {
      cat(sprintf("    SNR = %.2f ...\n", s))
      c(list(SNR = s), .t3_point(m, s, CFG$rho))
    })
    cat(sprintf("\n  Results — %s:\n", m$label))
    .print_table(res, "SNR")
  }
...
if (run_part_3) {
  ...
  cat(strrep("=", 70), "\n")
  cat("HT Equi-Blocks GVS MC — Part 3: rho sweep\n")
  cat(sprintf("tFDR=%.2f  SNR=%.2f  t(%d)  n=%d  p=%d  %d MC\n",
              CFG$tFDR, CFG$snr, CFG$df, CFG$n, CFG$p, CFG$num_MC))
  cat(strrep("=", 70), "\n\n")
  ...
  for (m in GVS_METHODS) {
    ...
    res <- lapply(rho_grid, function(rho_val) {
      cat(sprintf("    rho = %.2f ...\n", rho_val))
      c(list(rho = rho_val), .t3_point(m, CFG$snr, rho_val))
    })
    cat(sprintf("\n  Results — %s:\n", m$label))
...
if (run_part_4) {
  ...
  cat(strrep("=", 70), "\n")
  cat("HT Equi-Blocks GVS MC  |  Part 4: SNR x rho grid\n")
  cat(sprintf("n=%d  p=%d  %d MC  tFDR=%.2f  corr_max=%.2f  t(%d)\n",
              CFG$n, CFG$p, CFG$num_MC, CFG$tFDR, CFG$corr_max, CFG$df))
  cat(strrep("=", 70), "\n\n")
  ...
  for (m in GVS_METHODS) {
    ...
    for (i in seq_along(snr_grid)) {
      for (j in seq_along(rho_grid)) {
        ...
        r <- .t3_point(m, snr_grid[i], rho_grid[j])
