...
The demo is intended as a numerical sanity check and interface demonstration rather than a performance benchmark. It
mirrors the corresponding C++ example in
[cpp/ml_methods/svd/demo_mlm_svd_01/](../../../../cpp/ml_methods/svd/demo_mlm_svd_01/), replacing Eigen matrices with
...
The script then prints the dimensions of $\mathbf{U}$, $\mathbf{S}$, and $\mathbf{V}$, reports the relative
...
and lists the top $M$ singular values. As a Python-side addition (not in the C++ demo), it also verifies that the
full-rank decomposition ($M = p$) reconstructs $\mathbf{X}$ exactly up to floating-point error.
...
so that $p > 2n$ and the solver routes through the $n \times n$ Gram matrix $\mathbf{X}\mathbf{X}^\top$.
...
- The first scenario is the “Direct Path” for $n \ge p$ and the second the “Gram Path” for the wide-matrix regime
  $p > 2n$.
...
- The reconstruction reported by the demo is a **truncated** reconstruction using only the top $M$ singular components,
  ...
  variation.
- `SVDSolver.compute()` requires a `float64`, Fortran-ordered (column-major) NumPy array (`np.asfortranarray`).
