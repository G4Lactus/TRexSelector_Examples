# ==============================================================================
# demo_ts_10_tncgmp.py
# ==============================================================================
#
# Demonstration of the T-NCGMP (Terminating Norm-Corrective Generalized Matching Pursuit) solver.
#
# Mirrors cpp/tsolvers/demo_ts_10_tncgmp/demo_ts_10_tncgmp.cpp.
#
# Parts:
#   Demo 1: Basic T-NCGMP with early stopping (internal normalization),
...
def demo_early_stopping(high_dim, rnd_coef, T_stop):
    print_section_header("Demo 1: Basic T-NCGMP with Early Stopping")
    ...
    solver = TNCGMP_Solver(X, D, y, NCGMPVariant.LineSearch, normalize=True, intercept=True,
                           verbose=True)
...
def demo_with_external_normalizer(high_dim, rnd_coef, T_stop):
    print_section_header("Demo 2: T-NCGMP with External Normalization")
...
def demo_serialization():
    print_section_header("Demo 3: T-NCGMP Serialization & Warm Start")
    ...
    solver_ref = TNCGMP_Solver(X, D, y, NCGMPVariant.LineSearch, normalize=True, intercept=True,
                               verbose=True)
    solver_ref.executeStep(T_stop_final, early_stop=True)
    print(f"\u2713 Reference completed with {solver_ref.getNumSteps()} steps\n")

    filename = os.path.join(_THIS_DIR, "tncgmp_checkpoint.bin")

    # STEP 1: Run partial path and save checkpoint.
    solver1 = TNCGMP_Solver(X, D, y, NCGMPVariant.LineSearch, normalize=True, intercept=True,
                            verbose=True)
    ...
    solver2 = TNCGMP_Solver(X, D, y, NCGMPVariant.LineSearch, normalize=True, intercept=True,
                            verbose=True)
...
def demo_memory_mapped(high_dim, rnd_coef, T_stop):
    print_section_header("Demo 4: T-NCGMP with Memory-Mapped Data")
    ...
    X_file = os.path.join(_THIS_DIR, "demo_tncgmp_X.bin")
    D_file = os.path.join(_THIS_DIR, "demo_tncgmp_D.bin")
    y_file = os.path.join(_THIS_DIR, "demo_tncgmp_y.bin")
    ...
    try:
        ...
        print("Running T-NCGMP on memory-mapped data...")
        solver = TNCGMP_Solver(X_mmap.to_numpy(), D_mmap.to_numpy(), y_mmap.to_numpy(),
                               NCGMPVariant.LineSearch, normalize=True, intercept=True, verbose=True)
        solver.executeStep(T_stop, early_stop=True)
        print("\u2713 T-NCGMP completed\n")
...
if __name__ == "__main__":
    print_section_header("T-NCGMP Demo Suite")
