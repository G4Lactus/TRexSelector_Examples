# Demo: T-NCGMP (Norm-Corrective Generalized Matching Pursuit) Solver (Python)
...
It mirrors `cpp/tsolvers/demo_ts_10_tncgmp`.
...
The solver class is `TNCGMP_Solver` from `trex_selector_neo.tsolvers.omp_based`; all parts construct it with the `NCGMPVariant.LineSearch` variant as an additional positional argument.
...
The second part applies external L2 normalization (`LpNormScaler` with `NormType.L2`) to both the signal matrix and the dummy matrix, centers the response manually, and then runs T-NCGMP with the same solver variant on the transformed data.
...
The third part runs a reference fit to a final stopping level, then separately runs a shorter partial fit, saves the solver state to disk, reloads it, and continues from the checkpoint to the same final stopping level. This part runs its solvers with `verbose=True`, so the step-by-step progress is printed as well.

Unlike cpp, which uses a static `load(filename, X, D)` factory, the Python binding hydrates an existing solver instance: a new `TNCGMP_Solver` is constructed with the same `X`, `D`, `y`, and variant, and then `solver.load(path)` restores the checkpointed state.
...
The fourth part writes $X$, $D$, and $y$ to disk with `numpy_to_memmap()` and runs T-NCGMP on the zero-copy mmap views (via `.to_numpy()`).
...
```bash
.venv/bin/python Python/tsolvers/demo_ts_10_tncgmp/demo_ts_10_tncgmp.py
```
...
- All parts use the `NCGMPVariant.LineSearch` solver variant.
...
- The serialization example writes a temporary checkpoint file `tncgmp_checkpoint.bin` into the demo folder and removes it afterward.
- The memory-mapped example writes `demo_tncgmp_{X,D,y}.bin` into the demo folder, runs T-NCGMP on those mapped views, and removes those files at the end.
