...
from trex_selector_neo.tsolvers.omp_based import TMP_Solver
...
def demo_early_stopping(high_dim, rnd_coef, T_stop):
    ...
    solver = TMP_Solver(X, D, y, normalize=True, intercept=True,
                        verbose=True)
...
def demo_with_external_normalizer(high_dim, rnd_coef, T_stop):
    ...
    solver = TMP_Solver(X, D, y, normalize=False, intercept=False,
                        verbose=True)
    solver.executeStep(T_stop, early_stop=True)
...
def demo_serialization():
    ...
    solver_ref = TMP_Solver(X, D, y, normalize=True, intercept=True,
                            verbose=False)
    solver_ref.executeStep(T_stop_final, early_stop=True)
    print(f"\u2713 Reference completed with {solver_ref.getNumSteps()} steps\n")

    filename = os.path.join(_THIS_DIR, "tmp_checkpoint.bin")

    # STEP 1: Run partial path and save checkpoint.
    solver1 = TMP_Solver(X, D, y, normalize=True, intercept=True,
                         verbose=False)
    ...
    solver2 = TMP_Solver(X, D, y, normalize=True, intercept=True,
                         verbose=False)
...
def demo_memory_mapped(high_dim, rnd_coef, T_stop):
    ...
    X_file = os.path.join(_THIS_DIR, "demo_tmp_X.bin")
    D_file = os.path.join(_THIS_DIR, "demo_tmp_D.bin")
    y_file = os.path.join(_THIS_DIR, "demo_tmp_y.bin")
    ...
    try:
        ...
        print("Running T-MP on memory-mapped data...")
        solver = TMP_Solver(X_mmap.to_numpy(), D_mmap.to_numpy(), y_mmap.to_numpy(),
                            normalize=True, intercept=True, verbose=True)
        solver.executeStep(T_stop, early_stop=True)
        print("\u2713 T-MP completed\n")
