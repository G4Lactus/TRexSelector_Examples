# ==============================================================================
# demo_ts_03_tstepwise.py
# ==============================================================================
#
# Demonstration of the T-Stepwise (Terminating Forward Stepwise Regression) solver.
#
# Mirrors cpp/tsolvers/demo_ts_03_tstepwise/demo_ts_03_tstepwise.cpp.
#
# Parts:
#   Demo 1: Basic T-Stepwise with early stopping (internal normalization),
...
def demo_early_stopping(high_dim, rnd_coef, T_stop):
    print_section_header("Demo 1: Basic T-Stepwise with Early Stopping")
    ...
    true_coefs = [-0.4, -0.2, -0.8, 1.1, 2.5] if rnd_coef else [0.5, 1.0, 1.0, 1.0, 0.5]
    snr = 0.1
    ...
    solver = TSTEPWISE_Solver(X, D, y, normalize=True, intercept=True,
                              verbose=True)
...
def demo_with_external_normalizer(high_dim, rnd_coef, T_stop):
    print_section_header("Demo 2: T-Stepwise with External Normalization")
    ...
    solver = TSTEPWISE_Solver(X, D, y, normalize=False, intercept=False,
                              verbose=True)
    solver.executeStep(T_stop, early_stop=True)
...
def demo_serialization():
    ...
    solver_ref = TSTEPWISE_Solver(X, D, y, normalize=True, intercept=True,
                                  verbose=False)
    solver_ref.executeStep(T_stop_final, early_stop=True)
    print(f"\u2713 Reference completed with {solver_ref.getNumSteps()} steps\n")

    filename = os.path.join(_THIS_DIR, "tstepwise_checkpoint.bin")

    # STEP 1: Run partial path and save checkpoint.
    solver1 = TSTEPWISE_Solver(X, D, y, normalize=True, intercept=True,
                               verbose=False)
    ...
    solver2 = TSTEPWISE_Solver(X, D, y, normalize=True, intercept=True,
                               verbose=False)
...
def demo_memory_mapped(high_dim, rnd_coef, T_stop):
    print_section_header("Demo 4: T-Stepwise with Memory-Mapped Data")
    ...
    X_file = os.path.join(_THIS_DIR, "demo_tstepwise_X.bin")
    D_file = os.path.join(_THIS_DIR, "demo_tstepwise_D.bin")
    y_file = os.path.join(_THIS_DIR, "demo_tstepwise_y.bin")
    ...
    try:
        ...
        print("Running T-Stepwise on memory-mapped data...")
        solver = TSTEPWISE_Solver(X_mmap.to_numpy(), D_mmap.to_numpy(), y_mmap.to_numpy(),
                                  normalize=True, intercept=True, verbose=True)
        solver.executeStep(T_stop, early_stop=True)
        print("\u2713 T-Stepwise completed\n")
...
if __name__ == "__main__":
    print_section_header("T-Stepwise Demo Suite")
