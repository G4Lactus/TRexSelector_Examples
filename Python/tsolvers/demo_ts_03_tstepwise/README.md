...
It mirrors `cpp/tsolvers/demo_ts_03_tstepwise`.
...
The solver class is `TSTEPWISE_Solver` from `trex_selector_neo.tsolvers.lars_based`.
...
This part deliberately uses a hard regime: `snr = 0.1` and `true_coefs = [0.5, 1, 1, 1, 0.5]`, so only partial support recovery should be expected.
...
The second part applies external L2 normalization (`LpNormScaler` with `NormType.L2`) to both the signal matrix and the dummy matrix, centers the response manually, and then runs T-Stepwise with internal normalization disabled.
...
Unlike cpp, which uses a static `load(filename, X, D)` factory, the Python binding hydrates an existing solver instance: a new `TSTEPWISE_Solver` is constructed with the same `X`, `D`, and `y`, and then `solver.load(path)` restores the checkpointed state.
...
The fourth part writes $X$, $D$, and $y$ to disk with `numpy_to_memmap()` and runs T-Stepwise on the zero-copy mmap views (via `.to_numpy()`).
...
```bash
.venv/bin/python Python/tsolvers/demo_ts_03_tstepwise/demo_ts_03_tstepwise.py
```
...
- whether the selected variables recover the known true support reasonably well (in Part 1, only partially, by design),
...
- The first demo uses a lower signal-to-noise ratio (`snr = 0.1`) and a different coefficient pattern than the external-normalization and memory-mapped examples, so behavior across sections is not meant to be numerically identical.
- The thread count is set to 6 via `trex_selector_neo.utils.set_num_threads`, mirroring the cpp OpenMP setup.
- The serialization example writes a temporary checkpoint file `tstepwise_checkpoint.bin` into the demo folder and removes it afterward.
- The memory-mapped example writes `demo_tstepwise_{X,D,y}.bin` into the demo folder, runs T-Stepwise on those mapped views, and removes those files at the end.
