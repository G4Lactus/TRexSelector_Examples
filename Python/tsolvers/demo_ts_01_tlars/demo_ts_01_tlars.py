...
# ==============================================================================
#
# Demonstration of the T-LARS (Terminating Least Angle Regression) solver.
#
# Mirrors cpp/tsolvers/demo_ts_01_tlars/demo_ts_01_tlars.cpp.
#
# Parts:
#   Demo 1: Basic T-LARS with early stopping (internal normalization),
...
def demo_early_stopping(high_dim, rnd_coef, T_stop):
    print_section_header("Demo 1: Basic T-LARS with Early Stopping")
    ...
    solver = TLARS_Solver(X, D, y, normalize=True, intercept=True,
                          verbose=True)
...
def demo_with_external_normalizer(high_dim, rnd_coef, T_stop):
    print_section_header("Demo 2: T-LARS with External Normalization")
    ...
    solver = TLARS_Solver(X, D, y, normalize=False, intercept=False,
                          verbose=True)
    solver.executeStep(T_stop, early_stop=True)
...
def demo_serialization():
    print_section_header("Demo 3: T-LARS Serialization & Warm Start")
    ...
    solver_ref = TLARS_Solver(X, D, y, normalize=True, intercept=True,
                              verbose=False)
    solver_ref.executeStep(T_stop_final, early_stop=True)
    print(f"\u2713 Reference completed with {solver_ref.getNumSteps()} steps\n")

    filename = os.path.join(_THIS_DIR, "tlars_checkpoint.bin")

    # STEP 1: Run partial path and save checkpoint.
    solver1 = TLARS_Solver(X, D, y, normalize=True, intercept=True,
                           verbose=False)
    ...
    solver2 = TLARS_Solver(X, D, y, normalize=True, intercept=True,
                           verbose=False)
...
def demo_memory_mapped(high_dim, rnd_coef, T_stop):
    print_section_header("Demo 4: T-LARS with Memory-Mapped Data")
    ...
    X_file = os.path.join(_THIS_DIR, "demo_tlars_X.bin")
    D_file = os.path.join(_THIS_DIR, "demo_tlars_D.bin")
    y_file = os.path.join(_THIS_DIR, "demo_tlars_y.bin")
    ...
    try:
        ...
        print("Running T-LARS on memory-mapped data...")
        solver = TLARS_Solver(X_mmap.to_numpy(), D_mmap.to_numpy(), y_mmap.to_numpy(),
                              normalize=True, intercept=True, verbose=True)
        solver.executeStep(T_stop, early_stop=True)
        print("\u2713 T-LARS completed\n")
...
if __name__ == "__main__":
    print_section_header("T-LARS Demo Suite")
