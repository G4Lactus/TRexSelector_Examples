# ==============================================================================
# demo_ts_02_tlasso.py
# ==============================================================================
#
# Demonstration of the T-LASSO (Terminating LASSO) solver.
#
# Mirrors cpp/tsolvers/demo_ts_02_tlasso/demo_ts_02_tlasso.cpp.
#
# Parts:
#   Demo 1: Basic T-LASSO with early stopping (internal normalization),
...
def demo_early_stopping(high_dim, rnd_coef, T_stop):
    print_section_header("Demo 1: Basic T-LASSO with Early Stopping")
    ...
    solver = TLASSO_Solver(X, D, y, normalize=True, intercept=True,
                           verbose=True)
...
def demo_with_external_normalizer(high_dim, rnd_coef, T_stop):
    print_section_header("Demo 2: T-LASSO with External Normalization")
    ...
    solver = TLASSO_Solver(X, D, y, normalize=False, intercept=False,
                           verbose=True)
    solver.executeStep(T_stop, early_stop=True)
...
def demo_serialization():
    ...
    solver_ref = TLASSO_Solver(X, D, y, normalize=True, intercept=True,
                               verbose=False)
    solver_ref.executeStep(T_stop_final, early_stop=True)
    print(f"\u2713 Reference completed with {solver_ref.getNumSteps()} steps\n")

    filename = os.path.join(_THIS_DIR, "tlasso_checkpoint.bin")

    # STEP 1: Run partial path and save checkpoint.
    solver1 = TLASSO_Solver(X, D, y, normalize=True, intercept=True,
                            verbose=False)
    ...
    solver2 = TLASSO_Solver(X, D, y, normalize=True, intercept=True,
                            verbose=False)
...
def demo_memory_mapped(high_dim, rnd_coef, T_stop):
    print_section_header("Demo 4: T-LASSO with Memory-Mapped Data")
    ...
    X_file = os.path.join(_THIS_DIR, "demo_tlasso_X.bin")
    D_file = os.path.join(_THIS_DIR, "demo_tlasso_D.bin")
    y_file = os.path.join(_THIS_DIR, "demo_tlasso_y.bin")
    ...
    try:
        ...
        print("Running T-LASSO on memory-mapped data...")
        solver = TLASSO_Solver(X_mmap.to_numpy(), D_mmap.to_numpy(), y_mmap.to_numpy(),
                               normalize=True, intercept=True, verbose=True)
        solver.executeStep(T_stop, early_stop=True)
        print("\u2713 T-LASSO completed\n")
...
if __name__ == "__main__":
    print_section_header("T-LASSO Demo Suite")
