...
# ==============================================================================
#
# Demonstration of the T-OLS (Terminating Orthogonal Least Squares) solver.
#
# Mirrors cpp/tsolvers/demo_ts_11_tools/demo_ts_11_tools.cpp.
#
# Parts:
#   Demo 1: Basic T-OLS with early stopping (internal normalization),
...
from trex_selector_neo.tsolvers.omp_based import TOOLS_Solver
...
def demo_early_stopping(high_dim, rnd_coef, T_stop):
    print_section_header("Demo 1: Basic T-OLS with Early Stopping")
    ...
    solver = TOOLS_Solver(X, D, y, normalize=True, intercept=True,
                          verbose=True)
...
def demo_with_external_normalizer(high_dim, rnd_coef, T_stop):
    print_section_header("Demo 2: T-OLS with External Normalization")
    ...
    solver = TOOLS_Solver(X, D, y, normalize=False, intercept=False,
                          verbose=True)
    solver.executeStep(T_stop, early_stop=True)
...
def demo_serialization():
    print_section_header("Demo 3: T-OLS Serialization & Warm Start")
    ...
    solver_ref = TOOLS_Solver(X, D, y, normalize=True, intercept=True,
                              verbose=False)
    solver_ref.executeStep(T_stop_final, early_stop=True)
    print(f"\u2713 Reference completed with {solver_ref.getNumSteps()} steps\n")

    filename = os.path.join(_THIS_DIR, "tools_checkpoint.bin")

    # STEP 1: Run partial path and save checkpoint.
    solver1 = TOOLS_Solver(X, D, y, normalize=True, intercept=True,
                           verbose=False)
    ...
    solver2 = TOOLS_Solver(X, D, y, normalize=True, intercept=True,
                           verbose=False)
...
def demo_memory_mapped(high_dim, rnd_coef, T_stop):
    print_section_header("Demo 4: T-OLS with Memory-Mapped Data")
    ...
    X_file = os.path.join(_THIS_DIR, "demo_tools_X.bin")
    D_file = os.path.join(_THIS_DIR, "demo_tools_D.bin")
    y_file = os.path.join(_THIS_DIR, "demo_tools_y.bin")
    ...
    try:
        ...
        print("Running T-OLS on memory-mapped data...")
        solver = TOOLS_Solver(X_mmap.to_numpy(), D_mmap.to_numpy(), y_mmap.to_numpy(),
                              normalize=True, intercept=True, verbose=True)
        solver.executeStep(T_stop, early_stop=True)
        print("\u2713 T-OLS completed\n")
...
if __name__ == "__main__":
    print_section_header("T-OLS Demo Suite")
