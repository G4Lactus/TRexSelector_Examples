...
# ==============================================================================
#
# Demonstration of the T-OMP (Terminating Orthogonal Matching Pursuit) solver.
#
# Mirrors cpp/tsolvers/demo_ts_06_tomp/demo_ts_06_tomp.cpp.
#
# Parts:
#   Demo 1: Basic T-OMP with early stopping (internal normalization),
...
from trex_selector_neo.tsolvers.omp_based import TOMP_Solver
...
def demo_early_stopping(high_dim, rnd_coef, T_stop):
    print_section_header("Demo 1: Basic T-OMP with Early Stopping")
    ...
    solver = TOMP_Solver(X, D, y, normalize=True, intercept=True,
                         verbose=True)
...
def demo_with_external_normalizer(high_dim, rnd_coef, T_stop):
    print_section_header("Demo 2: T-OMP with External Normalization")
    ...
    solver = TOMP_Solver(X, D, y, normalize=False, intercept=False,
                         verbose=True)
    solver.executeStep(T_stop, early_stop=True)
...
def demo_serialization():
    print_section_header("Demo 3: T-OMP Serialization & Warm Start")
    ...
    solver_ref = TOMP_Solver(X, D, y, normalize=True, intercept=True,
                             verbose=False)
    solver_ref.executeStep(T_stop_final, early_stop=True)
    print(f"\u2713 Reference completed with {solver_ref.getNumSteps()} steps\n")

    filename = os.path.join(_THIS_DIR, "tomp_checkpoint.bin")

    # STEP 1: Run partial path and save checkpoint.
    solver1 = TOMP_Solver(X, D, y, normalize=True, intercept=True,
                          verbose=False)
    ...
    solver2 = TOMP_Solver(X, D, y, normalize=True, intercept=True,
                          verbose=False)
...
def demo_memory_mapped(high_dim, rnd_coef, T_stop):
    print_section_header("Demo 4: T-OMP with Memory-Mapped Data")
    ...
    # The file stems are demo-specific so running several tsolver demos from
    # the same directory does not clobber each other's memory-mapped data
    # files.
    X_file = os.path.join(_THIS_DIR, "demo_tomp_X.bin")
    D_file = os.path.join(_THIS_DIR, "demo_tomp_D.bin")
    y_file = os.path.join(_THIS_DIR, "demo_tomp_y.bin")
    ...
    try:
        ...
        print("Running T-OMP on memory-mapped data...")
        solver = TOMP_Solver(X_mmap.to_numpy(), D_mmap.to_numpy(), y_mmap.to_numpy(),
                             normalize=True, intercept=True, verbose=True)
        solver.executeStep(T_stop, early_stop=True)
        print("\u2713 T-OMP completed\n")
...
if __name__ == "__main__":
    print_section_header("T-OMP Demo Suite")
