...
# ==============================================================================
#
# Demonstration of the T-GP (Terminating Gradient Pursuit) solver.
#
# Mirrors cpp/tsolvers/demo_ts_07_tgp/demo_ts_07_tgp.cpp.
#
# Parts:
#   Demo 1: Basic T-GP with early stopping (internal normalization),
...
import numpy as np
from trex_selector_neo.tsolvers.omp_based import TGP_Solver
...
def demo_early_stopping(high_dim, rnd_coef, T_stop):
    print_section_header("Demo 1: Basic T-GP with Early Stopping")
    ...
    solver = TGP_Solver(X, D, y, normalize=True, intercept=True,
                        verbose=True)
...
def demo_with_external_normalizer(high_dim, rnd_coef, T_stop):
    print_section_header("Demo 2: T-GP with External Normalization")
    ...
    solver = TGP_Solver(X, D, y, normalize=False, intercept=False,
                        verbose=True)
    solver.executeStep(T_stop, early_stop=True)
...
def demo_serialization():
    print_section_header("Demo 3: T-GP Serialization & Warm Start")
    ...
    solver_ref = TGP_Solver(X, D, y, normalize=True, intercept=True,
                            verbose=False)
    solver_ref.executeStep(T_stop_final, early_stop=True)
    print(f"\u2713 Reference completed with {solver_ref.getNumSteps()} steps\n")

    filename = os.path.join(_THIS_DIR, "tgp_checkpoint.bin")

    # STEP 1: Run partial path and save checkpoint.
    solver1 = TGP_Solver(X, D, y, normalize=True, intercept=True,
                         verbose=False)
    ...
    solver2 = TGP_Solver(X, D, y, normalize=True, intercept=True,
                         verbose=False)
...
def demo_memory_mapped(high_dim, rnd_coef, T_stop):
    print_section_header("Demo 4: T-GP with Memory-Mapped Data")
    ...
    X_file = os.path.join(_THIS_DIR, "demo_tgp_X.bin")
    D_file = os.path.join(_THIS_DIR, "demo_tgp_D.bin")
    y_file = os.path.join(_THIS_DIR, "demo_tgp_y.bin")
    ...
    try:
        ...
        print("Running T-GP on memory-mapped data...")
        solver = TGP_Solver(X_mmap.to_numpy(), D_mmap.to_numpy(), y_mmap.to_numpy(),
                            normalize=True, intercept=True, verbose=True)
        solver.executeStep(T_stop, early_stop=True)
        print("\u2713 T-GP completed\n")
...
if __name__ == "__main__":
    print_section_header("T-GP Demo Suite")
