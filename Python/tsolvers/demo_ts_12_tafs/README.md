# Demo: T-AFS (Adaptive Forward Stepwise) Solver (Python)
...
It mirrors `cpp/tsolvers/demo_ts_12_tafs`.
...
The solver class is `TAFS_Solver` from `trex_selector_neo.tsolvers.afs_based`; all parts construct it with `rho = 1.0` as an additional positional argument.
...
The second part applies external L2 normalization (`LpNormScaler` with `NormType.L2`) to both the signal matrix and the dummy matrix, centers the response manually, and then runs T-AFS with internal normalization disabled.
...
Unlike cpp, which uses a static `load(filename, X, D)` factory, the Python binding hydrates an existing solver instance: a new `TAFS_Solver` is constructed with the same `X`, `D`, `y`, and `rho`, and then `solver.load(path)` restores the checkpointed state.
...
The fourth part writes $X$, $D$, and $y$ to disk with `numpy_to_memmap()` and runs T-AFS on the zero-copy mmap views (via `.to_numpy()`).
...
```bash
.venv/bin/python Python/tsolvers/demo_ts_12_tafs/demo_ts_12_tafs.py
```
...
- All parts use the adaptive-selection parameter `rho = 1.0`.
...
- The serialization example writes a temporary checkpoint file `tafs_checkpoint.bin` into the demo folder and removes it afterward.
- The memory-mapped example writes `demo_tafs_{X,D,y}.bin` into the demo folder, runs T-AFS on those mapped views, and removes those files at the end.
