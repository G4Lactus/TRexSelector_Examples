...
# ==============================================================================
#
# Demonstration of the T-AFS (Terminating Adaptive Forward Stepwise) solver.
#
# Mirrors cpp/tsolvers/demo_ts_12_tafs/demo_ts_12_tafs.cpp.
#
# Parts:
#   Demo 1: Basic T-AFS with early stopping (internal normalization),
...
def demo_early_stopping(high_dim, rnd_coef, T_stop):
    ...
    solver = TAFS_Solver(X, D, y, rho, normalize=True, intercept=True,
                         verbose=True)
...
def demo_with_external_normalizer(high_dim, rnd_coef, T_stop):
    ...
    solver = TAFS_Solver(X, D, y, rho, normalize=False, intercept=False,
                         verbose=True)
    solver.executeStep(T_stop, early_stop=True)
...
def demo_serialization():
    ...
    solver_ref = TAFS_Solver(X, D, y, rho, normalize=True, intercept=True,
                             verbose=False)
    solver_ref.executeStep(T_stop_final, early_stop=True)
    print(f"\u2713 Reference completed with {solver_ref.getNumSteps()} steps\n")

    filename = os.path.join(_THIS_DIR, "tafs_checkpoint.bin")

    # STEP 1: Run partial path and save checkpoint.
    solver1 = TAFS_Solver(X, D, y, rho, normalize=True, intercept=True,
                          verbose=False)
    ...
    solver2 = TAFS_Solver(X, D, y, rho, normalize=True, intercept=True,
                          verbose=False)
...
def demo_memory_mapped(high_dim, rnd_coef, T_stop):
    ...
    X_file = os.path.join(_THIS_DIR, "demo_tafs_X.bin")
    D_file = os.path.join(_THIS_DIR, "demo_tafs_D.bin")
    y_file = os.path.join(_THIS_DIR, "demo_tafs_y.bin")
    ...
    try:
        ...
        print("Running T-AFS on memory-mapped data...")
        solver = TAFS_Solver(X_mmap.to_numpy(), D_mmap.to_numpy(), y_mmap.to_numpy(),
                             rho, normalize=True, intercept=True, verbose=True)
        solver.executeStep(T_stop, early_stop=True)
        print("\u2713 T-AFS completed\n")
...
if __name__ == "__main__":
    print_section_header("T-AFS Demo Suite")
