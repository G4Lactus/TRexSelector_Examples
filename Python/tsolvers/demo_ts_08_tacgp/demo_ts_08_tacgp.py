...
# ==============================================================================
#
# Demonstration of the T-ACGP (Terminating Approximate Conjugate Gradient Pursuit) solver.
#
# Mirrors cpp/tsolvers/demo_ts_08_tacgp/demo_ts_08_tacgp.cpp.
#
# Parts:
#   Demo 1: Basic T-ACGP with early stopping (internal normalization),
...
from trex_selector_neo.tsolvers.omp_based import TACGP_Solver
...
def demo_early_stopping(high_dim, rnd_coef, T_stop):
    print_section_header("Demo 1: Basic T-ACGP with Early Stopping")
    ...
    solver = TACGP_Solver(X, D, y, normalize=True, intercept=True,
                          verbose=True)
...
def demo_with_external_normalizer(high_dim, rnd_coef, T_stop):
    print_section_header("Demo 2: T-ACGP with External Normalization")
    ...
    solver = TACGP_Solver(X, D, y, normalize=False, intercept=False,
                          verbose=True)
    solver.executeStep(T_stop, early_stop=True)
...
def demo_serialization():
    print_section_header("Demo 3: T-ACGP Serialization & Warm Start")
    ...
    solver_ref = TACGP_Solver(X, D, y, normalize=True, intercept=True,
                              verbose=False)
    solver_ref.executeStep(T_stop_final, early_stop=True)
    print(f"\u2713 Reference completed with {solver_ref.getNumSteps()} steps\n")

    filename = os.path.join(_THIS_DIR, "tacgp_checkpoint.bin")

    # STEP 1: Run partial path and save checkpoint.
    solver1 = TACGP_Solver(X, D, y, normalize=True, intercept=True,
                           verbose=False)
    ...
    solver2 = TACGP_Solver(X, D, y, normalize=True, intercept=True,
                           verbose=False)
...
def demo_memory_mapped(high_dim, rnd_coef, T_stop):
    print_section_header("Demo 4: T-ACGP with Memory-Mapped Data")
    ...
    X_file = os.path.join(_THIS_DIR, "demo_tacgp_X.bin")
    D_file = os.path.join(_THIS_DIR, "demo_tacgp_D.bin")
    y_file = os.path.join(_THIS_DIR, "demo_tacgp_y.bin")
    ...
    try:
        ...
        print("Running T-ACGP on memory-mapped data...")
        solver = TACGP_Solver(X_mmap.to_numpy(), D_mmap.to_numpy(), y_mmap.to_numpy(),
                              normalize=True, intercept=True, verbose=True)
        solver.executeStep(T_stop, early_stop=True)
        print("\u2713 T-ACGP completed\n")
...
if __name__ == "__main__":
    print_section_header("T-ACGP Demo Suite")
