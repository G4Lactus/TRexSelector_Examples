# Demo: T-ACGP (Approximate Conjugate Gradient Pursuit) Solver (Python)
...
It mirrors `cpp/tsolvers/demo_ts_08_tacgp`.
...
The solver class is `TACGP_Solver` from `trex_selector_neo.tsolvers.omp_based`.
...
The second part applies external L2 normalization (`LpNormScaler` with `NormType.L2`) to both the signal matrix and the dummy matrix, centers the response manually, and then runs T-ACGP with internal normalization disabled.
...
Unlike cpp, which uses a static `load(filename, X, D)` factory, the Python binding hydrates an existing solver instance: a new `TACGP_Solver` is constructed with the same `X`, `D`, and `y`, and then `solver.load(path)` restores the checkpointed state.
...
The fourth part writes $X$, $D$, and $y$ to disk with `numpy_to_memmap()` and runs T-ACGP on the zero-copy mmap views (via `.to_numpy()`).
...
```bash
.venv/bin/python Python/tsolvers/demo_ts_08_tacgp/demo_ts_08_tacgp.py
```
...
- The serialization example writes a temporary checkpoint file `tacgp_checkpoint.bin` into the demo folder and removes it afterward.
- The memory-mapped example writes `demo_tacgp_{X,D,y}.bin` into the demo folder, runs T-ACGP on those mapped views, and removes those files at the end.
