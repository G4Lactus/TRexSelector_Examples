...
- The realized FDR ($\approx 0.13$–$0.14$) sits $\approx 0.03$ above the target and the legacy CRAN R reference ($0.100$). This is the long-known residual gap between the C++ core and the CRAN R implementation, investigated by the `cpp/trex_selector_methods/validation/trex_spca/` programs (suspects: the internal CD-ridge λ₂ CV vs. R's `cv.glmnet`, and dummy-generation details); `validation_trex_spca_06` confirms it is **not** a `TRexSPCASelector` artifact.
