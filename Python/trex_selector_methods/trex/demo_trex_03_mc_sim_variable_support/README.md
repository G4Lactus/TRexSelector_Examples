...
`tloop_stagnation_stop = True`, `tloop_max_stagnant_steps = 3`. With
...
`demo_trex_03_mc_sim_variable_support_results_n300_p1000_stagnation_window_3`
(aligned `.txt` table plus tidy `.csv` with FDR/TPR/AvgL/AvgT rows).

## Running

```bash
.venv/bin/python Python/trex_selector_methods/trex/demo_trex_03_mc_sim_variable_support/demo_trex_03_mc_sim_variable_support.py
```
