// ==============================================================================
// demo_trex_gvs_02_scattered.cpp
// ==============================================================================
/**
 * @file demo_trex_gvs_02_scattered.cpp
 *
 * @brief Single-trial sweep demo of T-Rex+GVS on the scattered-grouped DGP.
 *
 * @details
 *  Two sweep parts, each in its own function:
 *    demo_scattered_snr_sweep() — SNR in {0.1, 0.2, 0.5, 1.0, 2.0, 5.0},
 *                                 fixed sd_x = sqrt(0.01) (rho ≈ 0.99)
 *    demo_scattered_rho_sweep() — rho in {0.10, 0.20, ..., 0.90, 0.99},
 *                                 sd_x = sqrt((1-rho)/rho), fixed SNR = 2.0
 *
 *  For each grid point one trial is run (seed = 2026) for both EN and IEN.
 *
 *  DGP: 3 groups of 50 variables randomly scattered across p=500 (s = 150).
 *  Selector: K=20, tFDR=0.1, corr_max=0.98, single-linkage HAC, GCV lambda2.
 *
 *  Monte Carlo simulations are in sim_trex_gvs_02_scattered.cpp.
 */
// ==============================================================================
...
static void demo_scattered_snr_sweep()
{
    constexpr int         n          = 200;
    constexpr int         p          = 500;
    ...
    cdiag::print_section_header(
        "Part 1: SNR Sweep  |  Scattered-Grouped\n"
        "n=" + std::to_string(n) + "  p=" + std::to_string(p) +
        "  n_groups=3  group_size=50  sd_x=0.1 (rho ≈ 0.99)"
        "  tFDR=" + std::to_string(tFDR).substr(0, 3));

    for (double snr : snr_grid) {
        auto dat = make_scattered_grouped_dgp(n, p, n_groups, group_size,
                                               snr, sd_x, seed);
        ...
        const std::string lbl = "Scattered  snr=" + std::to_string(snr).substr(0, 3);
...
static void demo_scattered_rho_sweep()
{
    constexpr int         n          = 200;
    constexpr int         p          = 500;
    constexpr int         n_groups   = 3;
    ...
    cdiag::print_section_header(
        "Part 2: rho Sweep  |  Scattered-Grouped\n"
        "n=" + std::to_string(n) + "  p=" + std::to_string(p) +
    ...
    for (double rho : rho_grid) {
        ...
        const std::string lbl = "Scattered  rho=" + std::to_string(rho).substr(0, 4);
...
int main()
{
    ...
    demo_scattered_snr_sweep();
    demo_scattered_rho_sweep();


    std::cout << "Scattered-Grouped GVS demo complete.\n";
    return 0;
}
