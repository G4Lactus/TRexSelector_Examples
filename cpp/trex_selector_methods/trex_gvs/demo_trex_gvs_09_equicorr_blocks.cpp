// ==============================================================================
// demo_trex_gvs_09_equicorr_blocks.cpp
// ==============================================================================
/**
 * @file demo_trex_gvs_09_equicorr_blocks.cpp
 *
 * @brief Single-trial sweep demo: T-Rex+GVS on Gaussian equicorrelated blocks.
 *
 * @details
 *  Two sweep parts:
 *    demo_equicorr_snr_sweep() — SNR in {0.1,0.2,0.5,1.0,2.0,5.0},
 *                                fixed rho=0.75 (sd_x=sqrt(1/3))
 *    demo_equicorr_rho_sweep() — rho in {0.10,...,0.99}, fixed SNR=2.0
 *
 *  DGP: make_mixed_blocks_dgp (4 blocks {20,50,80,65}; s=150; block 3 trap).
 *  MC simulations are in sim_trex_gvs_09_equicorr_blocks.cpp.
 */
// ==============================================================================
...
static void demo_equicorr_snr_sweep()
{
    constexpr int         n        = 200;
    constexpr int         p        = 500;
    ...
    cdiag::print_section_header(
        "Part 1: SNR Sweep  |  Equicorr-Blocks\n"
        "n=" + std::to_string(n) + "  p=" + std::to_string(p) +
        "  s=150  rho=0.75  tFDR=" + std::to_string(tFDR).substr(0, 3));

    for (double snr : snr_grid) {
        auto dat = make_mixed_blocks_dgp(n, p, snr, sd_x, seed);
        ...
        const std::string lbl =
            "Equicorr-Blocks  snr=" + std::to_string(snr).substr(0, 3);
...
static void demo_equicorr_rho_sweep()
{
    constexpr int         n         = 200;
    constexpr int         p         = 500;
    ...
    cdiag::print_section_header(
        "Part 2: rho Sweep  |  Equicorr-Blocks\n"
        "n=" + std::to_string(n) + "  p=" + std::to_string(p) +
    ...
    for (double rho : rho_grid) {
        const double sd_x = std::sqrt((1.0 - rho) / rho);
        auto dat = make_mixed_blocks_dgp(n, p, fixed_snr, sd_x, seed);
        ...
        const std::string lbl =
            "Equicorr-Blocks  rho=" + std::to_string(rho).substr(0, 4);
...
int main()
{
    ...
    demo_equicorr_snr_sweep();
    demo_equicorr_rho_sweep();

    std::cout << "Equicorr-Blocks GVS demo complete.\n";
    return 0;
}
