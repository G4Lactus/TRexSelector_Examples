// ==============================================================================
// demo_trex_gvs_03_unequal_blocks.cpp
// ==============================================================================
/**
 * @file demo_trex_gvs_03_unequal_blocks.cpp
 *
 * @brief Single-trial sweep demo of T-Rex+GVS on the unequal-blocks DGP.
 *
 * @details
 *  Two sweep parts, each in its own function:
 *    demo_unequal_blocks_snr_sweep() — SNR in {0.1, 0.2, 0.5, 1.0, 2.0, 5.0},
 *                                      fixed sd_x = sqrt(0.01) (rho ≈ 0.99)
 *
 *    demo_unequal_blocks_rho_sweep() — rho in {0.10, 0.20, ..., 0.90, 0.99},
 *                                      sd_x = sqrt((1 - rho)/rho),
 *                                      fixed SNR = 2.0
 *
 *  For each grid point one trial is run (seed = 2026) for both EN and IEN.
 *
 *  DGP: 3 contiguous blocks {20, 50, 80}, all active (s = 150).
 *  Selector: K=20, tFDR=0.1, corr_max=0.98, single-linkage HAC, GCV lambda2.
 *
 *  Monte Carlo simulations are in sim_trex_gvs_03_unequal_blocks.cpp.
 */
// ==============================================================================
...
static void demo_unequal_blocks_snr_sweep()
{
    constexpr int         n        = 200;
    constexpr int         p        = 500;
    ...
    cdiag::print_section_header(
        "Part 1: SNR Sweep  |  Unequal-Blocks\n"
...
static void demo_unequal_blocks_rho_sweep()
{
    constexpr int         n         = 200;
    constexpr int         p         = 500;
    ...
    cdiag::print_section_header(
        "Part 2: rho Sweep  |  Unequal-Blocks\n"
        "n=" + std::to_string(n) + "  p=" + std::to_string(p) +
    ...
    for (double rho : rho_grid) {
        ...
        auto dat = make_unequal_blocks_dgp(n, p, fixed_snr, sd_x, seed);

        Eigen::Map<Eigen::MatrixXd> X_map(dat.X.data(),
                                           dat.X.rows(), dat.X.cols());
        Eigen::Map<Eigen::VectorXd> y_map(dat.y.data(), dat.y.rows());

        const std::string lbl = "Unequal-Blocks  rho=" +
...
int main()
{
    ...
    if (true) {
        demo_unequal_blocks_snr_sweep();
    }

    if (true) {
        demo_unequal_blocks_rho_sweep();
    }

    std::cout << "Unequal-Blocks GVS demo complete.\n";
    return 0;
}
