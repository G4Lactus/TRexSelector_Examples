// ==============================================================================
// sim_trex_gvs_05_random_blocks.cpp
// ==============================================================================
/**
 * @file sim_trex_gvs_05_random_blocks.cpp
 *
 * @brief MC simulation: T-Rex+GVS on the random-blocks DGP.
 *
 * @details
 *  Three parts, each in its own function:
 *    sim_random_blocks_part1() — SNR sweep (fixed sd_x = sqrt(0.01))
 *    sim_random_blocks_part2() — rho sweep (fixed SNR = 2.0, sd_x derived from rho)
 *    sim_random_blocks_part3() — 2-D SNR × rho grid
 *
 *  DGP: three active blocks {20,50,80} in random order with noise gaps (s=150).
...
static void sim_random_blocks_part1(const GVSSimConfig& cfg, const std::string& tag)
{
    const std::vector<double> snr_grid = {0.1, 0.2, 0.5, 1.0, 2.0, 5.0};

    GVSDGPFactory snr_fn = [&cfg](double snr, unsigned seed) {
        return make_random_blocks_dgp(cfg.n, cfg.p, snr, cfg.sd_x, seed);
    };
...
static void sim_random_blocks_part2(const GVSSimConfig& cfg, const std::string& tag)
{
    ...
    GVSRhoDGPFactory rho_fn = [&cfg](double rho, unsigned seed) {
        ...
        return make_random_blocks_dgp(cfg.n, cfg.p, cfg.snr, sd_x, seed);
    };
...
static void sim_random_blocks_part3(const GVSSimConfig& cfg, const std::string& tag)
{
    ...
    GVS2DDGPFactory dgp_2d = [&cfg](double snr, double rho, unsigned seed) {
        const double sd_x = std::sqrt((1.0 - rho) / rho);
        return make_random_blocks_dgp(cfg.n, cfg.p, snr, sd_x, seed);
    };
...
int main()
{
    ...
    const std::string scenario_tag = "Random-Blocks";

    // ==========================================================================
    //  Run simulations
    // ==========================================================================

    sim_random_blocks_part1(cfg, scenario_tag);
    sim_random_blocks_part2(cfg, scenario_tag);
    sim_random_blocks_part3(cfg, scenario_tag);
