// ==============================================================================
// demo_trex_gvs_04_mixed_blocks.cpp
// ==============================================================================
/**
 * @file demo_trex_gvs_04_mixed_blocks.cpp
 *
 * @brief Single-trial sweep demo of T-Rex+GVS on the mixed-blocks DGP.
 *
 * @details
 *  Two sweep parts, each in its own function:
 *
 *    demo_mixed_blocks_snr_sweep() — SNR in {0.1, 0.2, 0.5, 1.0, 2.0, 5.0},
 *                                    fixed sd_x = sqrt(0.01) (rho ≈ 0.99)
 *
 *    demo_mixed_blocks_rho_sweep() — rho in {0.10, 0.20, ..., 0.90, 0.99},
 *                                    sd_x = sqrt((1-rho)/rho), fixed SNR = 2.0
 *
 *  For each grid point one trial is run (seed = 2026) for both EN and IEN.
 *
 *  DGP: 4 blocks {20,50,80,65} in random order; first 3 active (s = 150).
 *  Selector: K=20, tFDR=0.1, corr_max=0.98, single-linkage HAC, GCV lambda2.
 *
 *  Monte Carlo simulations are in sim_trex_gvs_04_mixed_blocks.cpp.
 */
// ==============================================================================
...
static void demo_mixed_blocks_snr_sweep()
{
    constexpr int         n        = 200;
    constexpr int         p        = 500;
    ...
    cdiag::print_section_header(
        "Part 1: SNR Sweep  |  Mixed-Blocks\n"
        "n=" + std::to_string(n) + "  p=" + std::to_string(p) +
        "  blocks={20,50,80,65}  sd_x=0.1 (rho≈0.99)"
    ...
    for (double snr : snr_grid) {
        ...
        const std::string lbl = "Mixed-Blocks  snr=" + std::to_string(snr).substr(0, 3);
...
static void demo_mixed_blocks_rho_sweep()
{
    constexpr int         n         = 200;
    constexpr int         p         = 500;
    ...
    for (double rho : rho_grid) {
        ...
        const std::string lbl = "Mixed-Blocks  rho=" + std::to_string(rho).substr(0, 4);
...
int main()
{
    ...
    demo_mixed_blocks_snr_sweep();
    demo_mixed_blocks_rho_sweep();

    std::cout << "Mixed-Blocks GVS demo complete.\n";
    return 0;
}
