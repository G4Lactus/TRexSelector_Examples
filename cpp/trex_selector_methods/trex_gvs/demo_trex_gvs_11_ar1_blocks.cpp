// ==============================================================================
// demo_trex_gvs_11_ar1_blocks.cpp
// ==============================================================================
/**
 * @file demo_trex_gvs_11_ar1_blocks.cpp
 *
 * @brief Single-trial sweep demo: T-Rex+GVS on AR(1)-within-block DGP.
 *
 * @details
 *  Two sweep parts:
 *    demo_ar1_snr_sweep() — SNR in {0.1,0.2,0.5,1.0,2.0,5.0}, fixed rho=0.85
 *    demo_ar1_rho_sweep() — rho in {0.10,...,0.99}, fixed SNR=2.0
 *
 *  DGP: make_ar1_blocks_dgp (4 blocks {20,50,80,65}; AR(1) Sigma via Cholesky).
 *  MC simulations are in sim_trex_gvs_11_ar1_blocks.cpp.
 */
// ==============================================================================
...
static void demo_ar1_snr_sweep()
{
    constexpr int         n        = 200;
    constexpr int         p        = 500;
    ...
    cdiag::print_section_header(
        "Part 1: SNR Sweep  |  AR1-Blocks\n"
        "n=" + std::to_string(n) + "  p=" + std::to_string(p) +
        "  s=150  rho=0.85  tFDR=" + std::to_string(tFDR).substr(0, 3));

    for (double snr : snr_grid) {
        auto dat = make_ar1_blocks_dgp(n, p, snr, rho, seed);
        ...
        const std::string lbl =
            "AR1-Blocks  snr=" + std::to_string(snr).substr(0, 3);
...
static void demo_ar1_rho_sweep()
{
    constexpr int         n         = 200;
    constexpr int         p         = 500;
    ...
    for (double rho : rho_grid) {
        ...
        const std::string lbl =
            "AR1-Blocks  rho=" + std::to_string(rho).substr(0, 4);
...
int main()
{
    ...
    std::cout << "AR1-Blocks GVS demo complete.\n";
    return 0;
}
