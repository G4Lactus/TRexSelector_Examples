// ==============================================================================
// sim_trex_gvs_02_scattered.cpp
// ==============================================================================
/**
 * @file sim_trex_gvs_02_scattered.cpp
 *
 * @brief MC simulation: T-Rex+GVS on the scattered-grouped DGP.
 *
 * @details
 *  Three parts, each in its own function:
 *    sim_scattered_part1() — SNR sweep (fixed sd_x = sqrt(0.01))
 *    sim_scattered_part2() — rho sweep (fixed SNR = 2.0, sd_x derived from rho)
 *    sim_scattered_part3() — 2-D SNR × rho grid
 *
 *  DGP: 3 groups of 50 variables randomly scattered across p = 500 columns.
 *       All 50 variables per group active (s = 150).
 *  Selector: K=20, tFDR=0.1, corr_max=0.98, single-linkage HAC, CV_1SE lambda2.
 *  MC: 200 trials per grid point.
 *
 *  The single-run demo is in demo_trex_gvs_02_scattered.cpp.
 */
// ==============================================================================
...
static void sim_scattered_part1(const GVSSimConfig& cfg, const std::string& tag)
{
    ...
    GVSDGPFactory snr_fn = [&cfg](double snr, unsigned seed) {
        return make_scattered_grouped_dgp(cfg.n, cfg.p,
                                          n_groups, group_size,
                                          snr, cfg.sd_x, seed);
    };
    ...
    cdiag::print_section_header(
        ...
        "  n_groups=3  group_size=50  s=150  MC=" + std::to_string(cfg.num_MC) +
...
static void sim_scattered_part2(const GVSSimConfig& cfg, const std::string& tag)
{
    constexpr int n_groups   = 3;
    constexpr int group_size = 50;
...
static void sim_scattered_part3(const GVSSimConfig& cfg, const std::string& tag)
{
    constexpr int n_groups   = 3;
    constexpr int group_size = 50;
...
int main()
{
    ...
    cfg.n         = 200;
    cfg.p         = 500;
    cfg.sd_x      = 0.1;    // sqrt(0.01) => within-group rho ≈ 0.99
    cfg.tFDR      = 0.1;
    cfg.K         = 20;
    cfg.num_MC    = 200;
    cfg.base_seed = 2026;
    ...
    const std::string scenario_tag = "Scattered-Grouped";

    // ==========================================================================
    //  Run simulations
    // ==========================================================================

    sim_scattered_part1(cfg, scenario_tag);
    sim_scattered_part2(cfg, scenario_tag);
    sim_scattered_part3(cfg, scenario_tag);
