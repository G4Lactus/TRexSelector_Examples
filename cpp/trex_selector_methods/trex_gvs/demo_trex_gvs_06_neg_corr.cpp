// ==============================================================================
// demo_trex_gvs_06_neg_corr.cpp
// ==============================================================================
/**
 * @file demo_trex_gvs_06_neg_corr.cpp
 *
 * @brief Single-trial sweep demo of T-Rex+GVS on the negative-correlation DGP.
 *
 * @details
 *  Two sweep parts, each in its own function:
 *    demo_neg_corr_snr_sweep() — SNR in {0.1, 0.2, 0.5, 1.0, 2.0, 5.0},
 *                                fixed sd_x = sqrt(0.01) (rho ≈ 0.99)
 *    demo_neg_corr_rho_sweep() — rho in {0.10, 0.20, ..., 0.90, 0.99},
 *                                sd_x = sqrt((1-rho)/rho), fixed SNR = 2.0
 *
 *  For each grid point one trial is run (seed = 2026) for both EN and IEN.
 *
 *  DGP: active group (0-99, s=100) + inactive trap (100-199).
 *  Selector: K=20, tFDR=0.1, corr_max=0.98, single-linkage HAC, GCV lambda2.
 *
 *  Monte Carlo simulations are in sim_trex_gvs_06_neg_corr.cpp.
 */
// ==============================================================================
...
static void demo_neg_corr_snr_sweep()
{
    constexpr int         n        = 200;
    constexpr int         p        = 500;
...
static void demo_neg_corr_rho_sweep()
{
    constexpr int         n         = 200;
    constexpr int         p         = 500;
    ...
    for (double rho : rho_grid) {
        ...
        auto dat = make_neg_corr_dgp(n, p, fixed_snr, sd_x, seed);

        Eigen::Map<Eigen::MatrixXd> X_map(dat.X.data(),
                                           dat.X.rows(), dat.X.cols());
        Eigen::Map<Eigen::VectorXd> y_map(dat.y.data(), dat.y.rows());

        const std::string lbl = "Neg-Corr  rho=" + std::to_string(rho).substr(0, 4);
...
int main()
{
    ...
    demo_neg_corr_snr_sweep();
    demo_neg_corr_rho_sweep();

    std::cout << "Neg-Corr GVS demo complete.\n";
    return 0;
}
