// ==============================================================================
// sim_trex_gvs_10_t3_blocks.cpp
// ==============================================================================
/**
 * @file sim_trex_gvs_10_t3_blocks.cpp
 *
 * @brief MC simulation: T-Rex+GVS on heavy-tailed equicorrelated blocks.
 *
 * @details
 *  Three parts, each in its own function:
 *    sim_t3_part1() — SNR sweep (fixed rho = 0.75)
 *    sim_t3_part2() — rho sweep (fixed SNR = 2.0)
 *    sim_t3_part3() — 2-D SNR × rho grid
 *
 *  DGP: make_t3_equicorr_blocks_dgp — same 4-block structure as Equicorr-Blocks
 *       but with scaled Student-t(3) latent factors and response noise.
 *       s=150, blocks 0-2 active (beta=3), block 3 is inactive t(3) trap.
...
static void sim_t3_part1(const GVSSimConfig& cfg, const std::string& tag)
{
    const std::vector<double> snr_grid = {0.1, 0.2, 0.5, 1.0, 2.0, 5.0};
    constexpr double fixed_rho = 0.75;
...
static void sim_t3_part2(const GVSSimConfig& cfg, const std::string& tag)
{
...
static void sim_t3_part3(const GVSSimConfig& cfg, const std::string& tag)
{
...
int main()
{
    ...
    cfg.n         = 200;
    cfg.p         = 500;
    cfg.sd_x      = 0.0;    // not used; rho passed directly to DGP
    cfg.tFDR      = 0.1;
    cfg.K         = 20;
    cfg.num_MC    = 200;
    cfg.base_seed = 2026;
    ...
    const std::string scenario_tag = "t3-Blocks";

    sim_t3_part1(cfg, scenario_tag);
    sim_t3_part2(cfg, scenario_tag);
    sim_t3_part3(cfg, scenario_tag);
