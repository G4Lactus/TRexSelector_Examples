...
"$venv_python" "$plotter" "$data/gvs_Hastie_2d.csv" \
  --title 'T-Rex+GVS on Hastie equicorrelated blocks: SNR $\times$ $\rho$ sweep' \
  --tfdr "$tfdr" "$@"
"$venv_python" "$plotter" "$data/gvs_Hastie_lambda2_method.csv" \
  --title 'T-Rex+GVS on Hastie: $\lambda_2$ selection method (SNR=2, $\rho$=0.7)' \
  --param-label '$\lambda_2$ selection method' --tfdr "$tfdr" "$@"
"$venv_python" "$plotter" "$data/gvs_Hastie_hc_linkage.csv" \
  --title 'T-Rex+GVS on Hastie: HAC linkage (SNR=2, $\rho$=0.7)' \
  --param-label 'HAC linkage' --tfdr "$tfdr" "$@"
