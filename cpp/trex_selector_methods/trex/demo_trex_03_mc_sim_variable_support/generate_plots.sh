...
csv="$here/simulation_results/data/demo_trex_03_mc_sim_variable_support_trex_results_n300_p1000_stagnation_window_5.csv"
if [[ $# -gt 0 && "$1" != -* ]]; then
  csv="$1"
  shift
fi
...
exec "$venv_python" "$here/../trex_plt_utils.py" "$csv" \
  --title 'T-Rex Monte Carlo simulation ($n=300$, $p=1000$, variable support, 10 runs)' \
  --legend-title 'Solver' \
  "$@"
