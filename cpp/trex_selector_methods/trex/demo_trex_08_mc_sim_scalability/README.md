# Demo 08: Scalability Benchmark (Placeholder)

## Status

**Work in progress**: This demo folder is reserved for a future large-scale scalability benchmark. The source file `demo_trex_08_mc_sim_scalability.cpp` is currently an **empty placeholder**, and its `add_demo_target(...)` entry is **commented out** in `../CMakeLists.txt`, so nothing is built or run for this demo yet. Everything below is a proposed design, not implemented behaviour.
