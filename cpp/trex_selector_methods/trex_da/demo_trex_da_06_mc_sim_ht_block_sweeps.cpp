    ...
    for (const auto& [scen_str, h_noise] : all_scenarios()) {
        for (const auto& [lnk_str, lnk_val] : all_linkages()) {
            ...
            std::string title = "HT-block Q sweep  n=" + std::to_string(n)
                ...
                + "  nu=" + std::to_string(nu) + "  p=M*Q varies  noise=";
            title += scen_str; title += "  linkage="; title += lnk_str;
    ...
    for (const auto& [scen_str, h_noise] : all_scenarios()) {
        for (const auto& [lnk_str, lnk_val] : all_linkages()) {
            ...
            std::string title = "HT-block M sweep  n=" + std::to_string(n)
                + "  Q=" + std::to_string(Q)
                + "  rho=" + std::to_string(rho) + "  SNR=" + std::to_string(snr)
                + "  nu=" + std::to_string(nu) + "  p=M*Q and s=M vary  noise=";
            title += scen_str; title += "  linkage="; title += lnk_str;
...
int main() {
    ...
    if (false) demo_ht_block_mc_snr_sweep();
    if (false) demo_ht_block_mc_rho_sweep();
    if (false) demo_ht_block_mc_q_sweep();
    if (false) demo_ht_block_mc_m_sweep();
    if (false) demo_ht_block_mc_tfdr_sweep();
    if (false) demo_ht_block_mc_linkage_sweep();
