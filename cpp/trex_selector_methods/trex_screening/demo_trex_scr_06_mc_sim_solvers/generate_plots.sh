...
for csv in "${csvs[@]}"; do
  ...
  case "$stem" in
    scr_solvers_snr_*)       title='Screen-TRex solver backends (TLARS/TAFS/TOMP): TPR/FDR vs SNR' ;;
    scr_solver_method_snr_*) title='Screen-TRex solver x method grid: TPR/FDR vs SNR' ;;
    *) title="$stem" ;;
  esac
