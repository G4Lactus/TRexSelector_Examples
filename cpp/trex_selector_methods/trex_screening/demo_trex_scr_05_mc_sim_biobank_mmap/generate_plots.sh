...
for csv in "${csvs[@]}"; do
  ...
  case "$stem" in
    scr_biobank_mmap_multi_*) title='Biobank Screen-TRex (memory-mapped, 5 phenotypes): routing vs SNR' ;;
    scr_biobank_mmap_snr_*)   title='Biobank Screen-TRex (memory-mapped): adaptive routing vs SNR' ;;
    *) title="$stem" ;;
  esac
