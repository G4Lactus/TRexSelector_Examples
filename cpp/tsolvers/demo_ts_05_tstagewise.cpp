// ============================================================================
// demo_ts_05_tstagewise.cpp
// ============================================================================
/**
 * @file demo_ts_05_tstagewise.cpp
 *
 * @brief Demonstration of T-Stagewise (Terminating Incremental Forward
 *        Stagewise Regression) solver.
 *
...
// ============================================================================
// Demo 1: Basic T-Stagewise with Early Stopping
// ============================================================================

void demo_TSTAGEWISE_early_stopping(bool high_dim, bool rnd_coef, std::size_t T_stop) {

    // Print Demo Header
    cdiagnost::print_section_header("Demo 1: Basic T-Stagewise with Early Stopping");
    ...
    // Run T-Stagewise solver with internal normalization
    tsolvers::TSTAGEWISE_Solver tstagewise(
        X_map,
        D_map,
        y_map,
        /*normalize=*/true,
    ...
    tstagewise.executeStep(T_stop, /*early_stop=*/true);

    // Results
    // -----------------------------
    // Print selection path
    cdiagnost::print_selection(tstagewise, true_support);

    // Print selection quality measures
    cdiagnost::print_selection_quality(tstagewise, true_support);

    // T-Stagewise specific diagnostics
    std::cout << "\nT-Stagewise Diagnostics:\n";
    std::cout << "  Removals: " << tstagewise.getNumRemovals() << "\n";
    std::cout << "  Cycling ratio: " << std::fixed << std::setprecision(4)
              << tstagewise.getCyclingRatio() << "\n";
...
void demo_TSTAGEWISE_with_external_normalizer(bool high_dim, bool rnd_coef, std::size_t T_stop) {

    // Print Demo Header
    cdiagnost::print_section_header("=== Demo 2: T-Stagewise with External Normalization ===");
...
void demo_TSTAGEWISE_serialization() {

    // Print Demo Header
    cdiagnost::print_section_header("=== Demo 3: T-Stagewise Serialization & Warm Start ===");
    ...
    tsolvers::TSTAGEWISE_Solver solver_ref(
        X_map,
        D_map,
        y_map,
        /*normalize=*/true,
    ...
    std::string filename = "tstagewise_checkpoint.bin";

    // --------- STEP 1: Run partial path and save checkpoint (scoped) ---------
    {
        tsolvers::TSTAGEWISE_Solver solver1(
            X_map,
            D_map,
            y_map,
            /*normalize=*/true,
    ...
    {
        tsolvers::TSTAGEWISE_Solver solver2 =
            tsolvers::TSTAGEWISE_Solver::load(filename, X_map, D_map);
...
void demo_memory_mapped(bool high_dim, bool rnd_coef, std::size_t T_stop) {
    ...
    // Print Demo Header
    cdiagnost::print_section_header("=== Demo 4: T-Stagewise with Memory-Mapped Data ===");
    ...
    const std::string X_file = "demo_tstagewise_X.bin";
    const std::string D_file = "demo_tstagewise_D.bin";
    const std::string y_file = "demo_tstagewise_y.bin";
    ...
    // Run T-Stagewise
    std::cout << "Running T-Stagewise on memory-mapped data...\n";
    tsolvers::TSTAGEWISE_Solver tstagewise(
        X_map,
        D_map,
        y_map,
    ...
    // Execute
    tstagewise.executeStep(T_stop, /*early_stop=*/true);
    std::cout << "✓ T-Stagewise completed\n\n";
...
int main() {
    ...
    std::cout << "\n";
    // Print header
    cdiagnost::print_section_header("T-Stagewise Demo Suite");
    ...
    try {
        ...
        if (true) {
            demo_TSTAGEWISE_early_stopping(/*high_dim=*/true, /*rnd_coef=*/false, /*T_stop=*/10);
            demo_TSTAGEWISE_early_stopping(/*high_dim=*/false, /*rnd_coef=*/false, /*T_stop=*/10);
        }

        // Demo 2: External normalization
        if (true) {
            demo_TSTAGEWISE_with_external_normalizer(/*high_dim=*/false, /*rnd_coef=*/false,
                                                     /*T_stop=*/10);
            demo_TSTAGEWISE_with_external_normalizer(/*high_dim=*/true, /*rnd_coef=*/false,
                                                     /*T_stop=*/10);
        }

        // Demo 3: Serialization
        if (true) {
            demo_TSTAGEWISE_serialization();
        }
