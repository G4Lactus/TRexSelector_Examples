...
// ============================================================================
/**
 * @file demo_ts_12_tafs.cpp
 *
 * @brief Demonstration of T-AFS (Terminating Adaptive Forward Stepwise) solver.
 *
...
namespace tsolvers = trex::tsolvers::linear_model::afs_based;
...
void demo_TAFS_early_stopping(bool high_dim, bool rnd_coef, std::size_t T_stop, double rho) {

    // Print Demo Header
    cdiagnost::print_section_header("Demo 1: Basic T-AFS with Early Stopping");
    ...
    // Run T-AFS solver with internal normalization
    tsolvers::TAFS_Solver tafs(
        X_map,
        D_map,
        y_map,
        rho,
    ...
    tafs.executeStep(T_stop, /*early_stop=*/true);

    // Results
    // -----------------------------
    // Print selection path
    cdiagnost::print_selection(tafs, true_support);

    // Print selection quality measures
    cdiagnost::print_selection_quality(tafs, true_support);
...
void demo_TAFS_with_external_normalizer(bool high_dim,
                                        bool rnd_coef,
                                        std::size_t T_stop,
                                        double rho) {

    // Print Demo Header
    cdiagnost::print_section_header("=== Demo 2: T-AFS with External Normalization ===");
...
void demo_TAFS_serialization(double rho) {

    // Print Demo Header
    cdiagnost::print_section_header("=== Demo 3: T-AFS Serialization & Warm Start ===");
    ...
    tsolvers::TAFS_Solver solver_ref(
        X_map,
        D_map,
        y_map,
        rho,
    ...
    std::string filename = "tafs_checkpoint.bin";

    // --------- STEP 1: Run partial path and save checkpoint (scoped) ---------
    {
        tsolvers::TAFS_Solver solver1(
            X_map,
            D_map,
            y_map,
            rho,
    ...
    {
        tsolvers::TAFS_Solver solver2 =
            tsolvers::TAFS_Solver::load(filename, X_map, D_map);
...
void demo_memory_mapped(bool high_dim, bool rnd_coef, std::size_t T_stop, double rho) {

    // Print Demo Header
    cdiagnost::print_section_header("=== Demo 4: T-AFS with Memory-Mapped Data ===");
    ...
    const std::string X_file = "demo_tafs_X.bin";
    const std::string D_file = "demo_tafs_D.bin";
    const std::string y_file = "demo_tafs_y.bin";
    ...
    // Run T-AFS
    std::cout << "Running T-AFS on memory-mapped data...\n";
    tsolvers::TAFS_Solver tafs(
        X_map,
        D_map,
        y_map,
...
int main() {
    ...
    std::cout << "\n";
    // Print header
    cdiagnost::print_section_header("T-AFS Demo Suite");
    ...
    try {
        ...
        if (true) {
            demo_TAFS_early_stopping(/*high_dim=*/true, /*rnd_coef=*/false,
                                     /*T_stop=*/10, /*rho=*/1.0);
            demo_TAFS_early_stopping(/*high_dim=*/false, /*rnd_coef=*/false,
                                     /*T_stop=*/10, /*rho=*/1.0);
        }

        // Demo 2: External normalization
        if (true) {
            demo_TAFS_with_external_normalizer(/*high_dim=*/false, /*rnd_coef=*/false,
                                               /*T_stop=*/10, /*rho=*/1.0);
            demo_TAFS_with_external_normalizer(/*high_dim=*/true, /*rnd_coef=*/false,
                                               /*T_stop=*/10, /*rho=*/1.0);
        }

        // Demo 3: Serialization
        if (true) {
            demo_TAFS_serialization(/*rho=*/1.0);
        }
