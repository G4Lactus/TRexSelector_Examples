...
// ============================================================================
/**
 * @file demo_ts_09_tmp.cpp
 *
 * @brief Demonstration of T-MP (Terminating Matching Pursuit) solver.
 *
...
void demo_TMP_early_stopping(bool high_dim, bool rnd_coef, std::size_t T_stop) {

    // Print Demo Header
    cdiagnost::print_section_header("Demo 1: Basic T-MP with Early Stopping");
    ...
    // Run T-MP solver with internal normalization
    tsolvers::TMP_Solver tmp(
        X_map,
        D_map,
        y_map,
        /*normalize=*/true,
    ...
    tmp.executeStep(T_stop, /*early_stop=*/true);

    // Results
    // -----------------------------
    // Print selection path
    cdiagnost::print_selection(tmp, true_support);

    // Print selection quality measures
    cdiagnost::print_selection_quality(tmp, true_support);
...
void demo_TMP_with_external_normalizer(bool high_dim, bool rnd_coef, std::size_t T_stop) {

    // Print Demo Header
    cdiagnost::print_section_header("=== Demo 2: T-MP with External Normalization ===");
...
void demo_TMP_serialization() {

    // Print Demo Header
    cdiagnost::print_section_header("=== Demo 3: T-MP Serialization & Warm Start ===");
    ...
    tsolvers::TMP_Solver solver_ref(
        X_map,
        D_map,
        y_map,
        /*normalize=*/true,
    ...
    std::string filename = "tmp_checkpoint.bin";

    // --------- STEP 1: Run partial path and save checkpoint (scoped) ---------
    {
        tsolvers::TMP_Solver solver1(
            X_map,
            D_map,
            y_map,
            /*normalize=*/true,
    ...
    {
        tsolvers::TMP_Solver solver2 =
            tsolvers::TMP_Solver::load(filename, X_map, D_map);
...
void demo_memory_mapped(bool high_dim, bool rnd_coef, std::size_t T_stop) {
    ...
    // Print Demo Header
    cdiagnost::print_section_header("=== Demo 4: T-MP with Memory-Mapped Data ===");
    ...
    const std::string X_file = "demo_tmp_X.bin";
    const std::string D_file = "demo_tmp_D.bin";
    const std::string y_file = "demo_tmp_y.bin";
    ...
    // Run T-MP
    std::cout << "Running T-MP on memory-mapped data...\n";
    tsolvers::TMP_Solver tmp(
        X_map,
        D_map,
        y_map,
...
int main() {
    ...
    std::cout << "\n";
    // Print header
    cdiagnost::print_section_header("T-MP Demo Suite");
    ...
    try {
        ...
        if (true) {
            demo_TMP_early_stopping(/*high_dim=*/true, /*rnd_coef=*/false, /*T_stop=*/10);
            demo_TMP_early_stopping(/*high_dim=*/false, /*rnd_coef=*/false, /*T_stop=*/10);
        }

        // Demo 2: External normalization
        if (true) {
            demo_TMP_with_external_normalizer(/*high_dim=*/false, /*rnd_coef=*/false,
                                              /*T_stop=*/10);
            demo_TMP_with_external_normalizer(/*high_dim=*/true, /*rnd_coef=*/false,
                                              /*T_stop=*/10);
        }

        // Demo 3: Serialization
        if (true) {
            demo_TMP_serialization();
        }
