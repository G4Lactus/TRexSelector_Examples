// ============================================================================
// demo_ts_10_tncgmp.cpp
// ============================================================================
/**
 * @file demo_ts_10_tncgmp.cpp
 *
 * @brief Demonstration of T-NCGMP (Terminating Norm-Corrective Generalized
...
// ============================================================================
// Demo 1: Basic T-NCGMP with Early Stopping
// ============================================================================

void demo_TNCGMP_early_stopping(bool high_dim, bool rnd_coef, std::size_t T_stop) {

    // Print Demo Header
    cdiagnost::print_section_header("Demo 1: Basic T-NCGMP with Early Stopping");
    ...
    // Run T-NCGMP solver with internal normalization
    ...
    tncgmp.executeStep(T_stop, /*early_stop=*/true);

    // Results
    // -----------------------------
    // Print selection path
    cdiagnost::print_selection(tncgmp, true_support);

    // Print selection quality measures
    cdiagnost::print_selection_quality(tncgmp, true_support);
...
void demo_TNCGMP_with_external_normalizer(bool high_dim, bool rnd_coef, std::size_t T_stop) {

    // Print Demo Header
    cdiagnost::print_section_header("=== Demo 2: T-NCGMP with External Normalization ===");
...
void demo_TNCGMP_serialization() {

    // Print Demo Header
    cdiagnost::print_section_header("=== Demo 3: T-NCGMP Serialization & Warm Start ===");
    ...
    tsolvers::TNCGMP_Solver solver_ref(
         X_map,
        D_map,
        y_map,
        tsolvers::NCGMPVariant::LineSearch,
    ...
    std::string filename = "tncgmp_checkpoint.bin";

    // --------- STEP 1: Run partial path and save checkpoint (scoped) ---------
    {
        tsolvers::TNCGMP_Solver solver1(
            X_map,
            D_map,
            y_map,
            tsolvers::NCGMPVariant::LineSearch,
    ...
    {
        tsolvers::TNCGMP_Solver solver2 =
            tsolvers::TNCGMP_Solver::load(filename, X_map, D_map);
...
void demo_memory_mapped(bool high_dim, bool rnd_coef, std::size_t T_stop) {
    ...
    // Print Demo Header
    cdiagnost::print_section_header("=== Demo 4: T-NCGMP with Memory-Mapped Data ===");
    ...
    const std::string X_file = "demo_tncgmp_X.bin";
    const std::string D_file = "demo_tncgmp_D.bin";
    const std::string y_file = "demo_tncgmp_y.bin";
    ...
    // Run T-NCGMP
    std::cout << "Running T-NCGMP on memory-mapped data...\n";
    tsolvers::TNCGMP_Solver tncgmp(
         X_map,
        D_map,
        y_map,
...
int main() {
    ...
    std::cout << "\n";
    // Print header
    cdiagnost::print_section_header("T-NCGMP Demo Suite");
    ...
    try {
        ...
        if (true) {
            demo_TNCGMP_early_stopping(/*high_dim=*/true, /*rnd_coef=*/false, /*T_stop=*/10);
            demo_TNCGMP_early_stopping(/*high_dim=*/false, /*rnd_coef=*/false, /*T_stop=*/10);
        }

        // Demo 2: External normalization
        if (true) {
            demo_TNCGMP_with_external_normalizer(/*high_dim=*/false, /*rnd_coef=*/false,
                                                 /*T_stop=*/10);
            demo_TNCGMP_with_external_normalizer(/*high_dim=*/true, /*rnd_coef=*/false,
                                                 /*T_stop=*/10);
        }

        // Demo 3: Serialization
        if (true) {
            demo_TNCGMP_serialization();
        }
