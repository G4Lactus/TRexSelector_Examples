// ============================================================================
// demo_ts_11_tools.cpp
// ============================================================================
/**
 * @file demo_ts_11_tools.cpp
 *
 * @brief Demonstration of T-OLS (Terminating Orthogonal Least Squares) solver.
 *
...
// ============================================================================
// Demo 1: Basic T-OLS with Early Stopping
// ============================================================================

void demo_TOOLS_early_stopping(bool high_dim, bool rnd_coef, std::size_t T_stop) {

    // Print Demo Header
    cdiagnost::print_section_header("Demo 1: Basic T-OLS with Early Stopping");
    ...
    // Run T-OLS solver with internal normalization
    tsolvers::TOOLS_Solver tools(
        X_map,
        D_map,
        y_map,
        /*normalize=*/true,
    ...
    tools.executeStep(T_stop, /*early_stop=*/true);

    // Results
    // -----------------------------
    // Print selection path
    cdiagnost::print_selection(tools, true_support);

    // Print selection quality measures
    cdiagnost::print_selection_quality(tools, true_support);
...
void demo_TOOLS_with_external_normalizer(bool high_dim, bool rnd_coef, std::size_t T_stop) {

    // Print Demo Header
    cdiagnost::print_section_header("=== Demo 2: T-OLS with External Normalization ===");
...
void demo_TOOLS_serialization() {

    // Print Demo Header
    cdiagnost::print_section_header("=== Demo 3: T-OLS Serialization & Warm Start ===");
    ...
    tsolvers::TOOLS_Solver solver_ref(
        X_map,
        D_map,
        y_map,
        /*normalize=*/true,
    ...
    std::string filename = "tools_checkpoint.bin";

    // --------- STEP 1: Run partial path and save checkpoint (scoped) ---------
    {
        tsolvers::TOOLS_Solver solver1(
            X_map,
            D_map,
            y_map,
            /*normalize=*/true,
    ...
    {
        tsolvers::TOOLS_Solver solver2 =
            tsolvers::TOOLS_Solver::load(filename, X_map, D_map);
...
void demo_memory_mapped(bool high_dim, bool rnd_coef, std::size_t T_stop) {
    ...
    // Print Demo Header
    cdiagnost::print_section_header("=== Demo 4: T-OLS with Memory-Mapped Data ===");
    ...
    const std::string X_file = "demo_tools_X.bin";
    const std::string D_file = "demo_tools_D.bin";
    const std::string y_file = "demo_tools_y.bin";
    ...
    // Run T-OLS
    std::cout << "Running T-OLS on memory-mapped data...\n";
    tsolvers::TOOLS_Solver tools(
        X_map,
        D_map,
        y_map,
...
int main() {
    ...
    std::cout << "\n";
    // Print header
    cdiagnost::print_section_header("T-OLS Demo Suite");
    ...
    try {
        ...
        if (true) {
            demo_TOOLS_early_stopping(/*high_dim=*/true, /*rnd_coef=*/false, /*T_stop=*/10);
            demo_TOOLS_early_stopping(/*high_dim=*/false, /*rnd_coef=*/false, /*T_stop=*/10);
        }

        // Demo 2: External normalization
        if (true) {
            demo_TOOLS_with_external_normalizer(/*high_dim=*/false, /*rnd_coef=*/false,
                                                /*T_stop=*/10);
            demo_TOOLS_with_external_normalizer(/*high_dim=*/true, /*rnd_coef=*/false,
                                                /*T_stop=*/10);
        }

        // Demo 3: Serialization
        if (true) {
            demo_TOOLS_serialization();
        }
